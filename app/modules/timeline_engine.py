...
def ready() -> bool:
    ...
    try:
        ...
        CausalLink(
            src_id="test1", dst_id="test2", weight=0.5, delay_days=10
        )
        MetaPattern(
            name="Test Pattern", score=0.8, description="Test açıklaması"
        )
        return True
...
def discover_triggers(
    window_a: List[EventNode], window_b: List[EventNode], min_score: float = 0.3
) -> List[MetaPattern]:
        ...
        for event_a in window_a:
            for event_b in window_b:
                ...
                astro_sim = calculate_astro_similarity(
                    event_a.astro_signature, event_b.astro_signature
                )
                ...
                category_bonus = 0.0
                if event_a.meta.get("category") == event_b.meta.get(
                    "category"
                ) and event_a.meta.get("category"):
                    category_bonus = 0.2
                ...
                # Toplam skor hesapla
                total_score = (
                    astro_sim * 0.6 + delay_consistency * 0.3 + category_bonus * 0.1
                )
